...
studies3_all = read.csv("../../../data/studies/_misc/studies3.csv", header = TRUE, sep = ",")

studies3_all$TWO_R = revalue(studies3_all$TWO_R, c("c4op" = "LL", "b4op" = "LH", "1" = "CT", "a3op" = "HL", "b3op" = "HH"))
studies3_all$TWO_R = factor(studies3_all$TWO_R, levels = c("HH", "HL", "LH", "CT", "LL"))

f3_df = studies3_all[
    studies3_all$First == "yes" &
    studies3_all$Input == "mouse" &
    studies3_all$ONE_O >= 430 &
    studies3_all$TWO_O >= 430 &
    studies3_all$ONE_F >= 20 &
    studies3_all$TWO_F >= 20
    ,]

v1 = data.frame(T = c(f3_df$ONE_T, f3_df$TWO_T), S = rep(factor(c("ONE", "TWO")), each = dim(f3_df)[1]), I = factor(c(as.character(f3_df$Input), as.character(f3_df$Input))), R = factor(c(as.character(f3_df$TWO_R), as.character(f3_df$TWO_R)), levels = c("HH", "HL", "CT", "LH", "LL")));
...
ggplot(f3_df, aes(y = ONE_T, colour = TWO_R)) +
    geom_boxplot() +
...
ggplot(f3_df, aes(y = TWO_T, colour = TWO_R)) +
    geom_boxplot() +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
    labs(x = "First Time", y = "Game 2 Touches - Game 1 Touches", title = "Game 2 touches - Game 1 Touches (n=2,161)", colour = "First Time")

ggplot(studies3_all, aes(First, DIFF, colour = First)) +
    geom_boxplot() +
    scale_color_grey() +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
    labs(x = "First Time", y = "Game 2 Touches - Game 1 Touches", title = "Game 2 touches - Game 1 Touches (n=2,161)", colour = "First Time")

ggplot(studies3_all[studies3_all$Input == "mouse" | studies3_all$Input == "touchpad" | studies3_all$Input == "touchscreen",], aes(Input, DIFF, colour = Input)) +
    geom_boxplot() +
    scale_color_grey() +
...
ggplot(studies3_all, aes(Input, ONE_T, colour = Input)) +
    geom_boxplot() +
    scale_color_grey() +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
    labs(x = "Devices", y = "Game 1 Touches", title = "Game 1 Touches By Device (n=2,161)", fill = "")

ggplot(f3_df, aes(x = Gender)) + geom_bar() + facet_grid(~TWO_R)
ggplot(f3_df, aes(x = Age)) + geom_bar() + facet_grid(~TWO_R)
ggplot(f3_df, aes(x = Browser)) + geom_bar() + facet_grid(~TWO_R)
