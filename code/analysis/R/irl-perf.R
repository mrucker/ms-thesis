...
p = read.csv("data/perf-irl.csv", header = TRUE, sep = ",")
...
v = ddply(p[p$n == 15,], .(s_f, algorithm), summarize, med = median(1 - value), avg = mean(1 - value), var = var(1 - value), se = sd(1 - value) / sqrt(length(value)))
ggplot(v, aes(s_f, avg, colour = algorithm)) +
    geom_point(aes(shape = algorithm), size = 3) +
    geom_line(aes(group = algorithm)) +
    geom_errorbar(aes(ymin = avg - se, ymax = avg + se)) +
