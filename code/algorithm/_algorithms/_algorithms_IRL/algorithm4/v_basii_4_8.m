function [v_i, v_p, v_b] = v_basii_4_8()
    
    LEVELS_N = [3 3 3 8 3 8 3 6];
...
function [vf] = v_feats(levels)
    ...
    vf = [
        ...
        val_to_d(a_m_l-1,a_m_n-1     );
        val_to_r(a_d_l-1,a_d_n/2, 4.5);
...
function vp = v_perms()
    ...
    [t_n_c, t_t_c, a_d_c, a_m_c, v_d_c, v_m_c, p_y_c, p_x_c] = ndgrid(t_n_i, t_t_i, a_d_i, a_m_i, v_d_i, v_m_i, p_y_i, p_x_i);
...
function ca = cursor_a_levels(states)
    ...
    min_d_level = 1;
    max_d_level = LEVELS_N(6);
    bin_d_size  = max_d_level*pi/2; 

    val_d = atan2(-states(6,:), states(5,:)) + pi;
    
    ca_m = bin_levels(vecnorm(states(5:6,:)), 25, 1, LEVELS_N(5));
    ca_d = bin_levels(val_d, bin_d_size, min_d_level, max_d_level);
