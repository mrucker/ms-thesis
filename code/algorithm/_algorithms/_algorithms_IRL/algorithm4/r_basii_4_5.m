function [state2identity, r_p, r_b] = r_basii_4_5()

    r_p = all_targ_perms();
    
    a_n = size(r_p,2);
    a_T = basii2indexes_T([3 3 8 4],[3 3 8 4]);
...
function rb = r_basii_feats(states)
    ...
    if any(sum(tou,1) == 0)
        ...
        target_features = double(vertcat((1:3)' == lox, (1:3)' == loy, (1:8)' == dir, (1:4)' == age));
    end
...
function tx = target_lox_features(states)
    ...
    lox_bs = [1/3, 2/3, inf] * states(9,1);
    
    [~, ti] = max(states(12:3:end,1)' <= lox_bs');
    tx      = repmat(ti', 1, size(states,2));
end

function ty = target_loy_features(states)

    loy_bs = [1/3, 2/3, inf] * states(10,1);
    
    [~, ti] = max(states(13:3:end,1)' <= loy_bs');
    ty      = repmat(ti', 1, size(states,2));
end
...
function perms = all_targ_perms()
    ...
    [age_c, dir_c, loy_c, lox_c] = ndgrid(age_i, dir_i, loy_i, lox_i);

    t_features = vertcat(lox_f(:,lox_c(:)), loy_f(:,loy_c(:)), dir_f(:,dir_c(:)), age_f(:,age_c(:)));
