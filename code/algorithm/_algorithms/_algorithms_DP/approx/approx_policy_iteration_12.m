function [Pf, Vf, Xs, Ys, Ks, As, f_time, b_time, v_time, a_time] = approx_policy_iteration_12(s_1, actions, reward, value_basii, trans_post, trans_pre, gamma, N, M, T, W)

    a_start = tic;

    g_row_r = [(gamma).^(0:T-1)     , zeros(1,W-1)];
    g_row_v = [zeros(1,T-1), gamma^T, zeros(1,W-1)];

    g_mat_r = zeros(W,size(g_row_r,2));
    g_mat_v = zeros(W,size(g_row_v,2));

    for w = 1:W
        g_mat_r(w, :) = circshift(g_row_r,w-1);
        g_mat_v(w, :) = circshift(g_row_v,w-1);
    end
    ...
    for n = 1:N 
        ...
        for m = 1:M
            ...
            X_v = X_v_m{m};
                ...
                for w = 1:W
                    i = coalesce_if_true(~isempty(X), @() all(X == X_b(:,w)));
                    y = g_mat_r(w,:) * X_r' + g_mat_v(w,:) * X_v' ;
