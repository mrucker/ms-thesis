...
documentClient.scan(params, function(err, data) {
	if (err) {
		...
		data.Items.forEach(function(item) {
			...
			var filePath = 'entries/participants/'+item.Id+'.json';
			var fileData = JSON.stringify(item);
			var fileFlag = {flag: 'wx+'};
			var errAsync = (err) => { 													
				if (err && err.code == 'EEXIST') console.log('participant ' + item.Id + ' already written');
				if (err && err.code != 'EEXIST') console.log(err); 
				if (!err                       ) console.log('participant ' + item.Id + ' freshly written');
			};
